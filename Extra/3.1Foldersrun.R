...
#countries=c("ref_pr_monthly","CCSM4", "MIROC", "CSIRO-Mk3", "IPSL-CM5A")
...
#database_creation(model="CRU", country = country, resol="25", maxstations=10000 )

# II. VARIABLES AND INDICES  CALCULATION --------------
  setwd(paste0(workdir,"/",country))
  #BaseDatosEstaciones with 3 columns: factor, num, num
  BaseDatosRegistros <- read.csv("BaseDatosRegistros.csv",sep=",")
  # Selecciona las columnas de enero a diciembre
  cols <- c("Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec")
  
  # Aplica la interpolación a cada columna
  BaseDatosRegistros[cols] <- lapply(BaseDatosRegistros[cols], function(x) na.approx(x, na.rm = FALSE))
  BaseDatosRegistros <- zero_correction(BaseDatosRegistros)
  elarray <- c(3101,3102,3103,3201,3202,3301,3302,3303,3304,4101,4102,4103,4104,4105,4106,4201,4202,4203,4204,4301,4302,4303,4304,4305)
  BaseDatosRegistros <- BaseDatosRegistros[BaseDatosRegistros$id_station == elarray,]
  
  write.csv("BaseDatosRegistros.csv",sep=",")
